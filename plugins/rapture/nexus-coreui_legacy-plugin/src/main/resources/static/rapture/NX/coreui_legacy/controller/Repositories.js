...
Ext.define('NX.coreui_legacy.controller.Repositories', {
  ...
  loadRelatedStores: function() {
    ...
    if (list) {
      ...
      NX.direct.coreui_legacy_Repository.readStatus(true, function(response) {
  ...
  deleteModel: function(model) {
    ...
    NX.direct.coreui_legacy_Repository.remove(model.getId(), function(response) {
      me.loadStore();
      if (Ext.isObject(response) && response.success) {
        NX.Messages.add({
  ...
  startStatusPolling: function() {
    ...
    me.statusProvider = Ext.direct.Manager.addProvider({
      type: 'polling',
      url: NX.direct.api.POLLING_URLS.coreui_legacy_Repository_readStatus,
      interval: 5000,
      baseParams: {
      },
      listeners: {
  ...
  blockProxy: function(model) {
    ...
    NX.direct.coreui_legacy_Repository.updateStatus(model.getId(), undefined, 'BLOCKED_MANUAL', function(response) {
      if (Ext.isObject(response) && response.success) {
        me.loadStore();
        NX.Messages.add({
  ...
  allowProxy: function(model) {
    ...
    NX.direct.coreui_legacy_Repository.updateStatus(model.getId(), undefined, 'ALLOW', function(response) {
      if (Ext.isObject(response) && response.success) {
        me.loadStore();
        NX.Messages.add({
  ...
  putOutOfService: function(model) {
    ...
    NX.direct.coreui_legacy_Repository.updateStatus(model.getId(), 'OUT_OF_SERVICE', undefined, function(response) {
      if (Ext.isObject(response) && response.success) {
        me.loadStore();
        NX.Messages.add({
  ...
  putInService: function(model) {
    ...
    NX.direct.coreui_legacy_Repository.updateStatus(model.getId(), 'IN_SERVICE', undefined, function(response) {
      if (Ext.isObject(response) && response.success) {
        me.loadStore();
        NX.Messages.add({
